...
#pragma omp parallel for
	...
	if(times[4][k]) printf("time[4][%d] = %f", k, times[4][k]);
		
	times[5][k] = mysecond();
#ifdef TUNED
        tuned_STREAM_Strided4();
#else
#pragma omp parallel for
	...
	if(times[5][k]) printf("time[5][%d] = %f", k, times[5][k]);
	}
